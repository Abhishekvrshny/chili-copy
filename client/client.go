...
func sendToServer() {
	...
	if fileSize < 1000 {
		...
		multiPartCopy(localFile, remoteFile, uint32(fileSize), returnMD5String)
		//singleCopy(localFile,remoteFile, uint64(fileSize), returnMD5String)
	}
...
func multiPartCopy(localFile string, remoteFile string, fileSize uint32, returnMD5String string) {
	conn := GetConnection(network, address)
